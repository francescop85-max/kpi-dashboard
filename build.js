...
function renderCards(K, total) {
  ...
  const narr = [
    ...
    kpi1.length ? kpi1.map(m=>m.method.replace(' Solicitation','')+': '+m.avg+'d ('+m.count+' PRs)').join(' · ') : 'No cycle data.',
...
function generateReport(){
  ...
  const rptCycBreakdown=K.kpi1.map(m=>{
    const sn=m.method.replace(' Solicitation','');
    const rc=m.avg>60?(m.avg>75?'#dc2626':'#d97706'):'#003366';
    return '<div style="display:flex;justify-content:space-between;align-items:baseline;padding:5px 0;border-bottom:1px solid #f1f5f9;">'
      ...
      +'<div><span style="font-size:19px;font-weight:800;color:'+rc+'">'+m.avg+'d</span>'
      +'<span style="font-size:10px;color:#94a3b8;margin-left:5px;">'+m.count+' PRs</span></div></div>';
  }).join('');
  const rptPrepBreakdown=K.kpi6.map(m=>{
    ...
    return '<div style="display:flex;justify-content:space-between;align-items:baseline;padding:5px 0;border-bottom:1px solid #f1f5f9;">'
      +'<span style="font-size:12px;color:#64748b;">'+sn+'</span>'
      +'<div><span style="font-size:19px;font-weight:800;color:'+rc+'">'+m.avg+'d</span>'
      +'<span style="font-size:10px;color:#94a3b8;margin-left:5px;">target: '+ptgt+'cd</span></div></div>';
  }).join('');

  // ── Narrative paragraphs ──────────────────────────────────────────
  const p1=(()=>{
    const cycBreakStr=K.kpi1.map(m=>m.method.replace(' Solicitation','')+': <strong>'+m.avg+'d</strong>').join(', ');
    const cycleMsg='Procurement cycle times differ significantly by modality — '+cycBreakStr+'. '
      +(avgCycle<=cycleTgt
        ?'The overall portfolio average of <strong>'+avgCycle+'d</strong> is within the '+cycleTgt+'-day benchmark.'
        :'The overall portfolio average of <strong>'+avgCycle+'d</strong> exceeds the '+cycleTgt+'-day benchmark, driven primarily by longer Formal Solicitation processes which are expected given their complexity.');
...
<div class="page">
  ...
  <div class="kpi-grid">
    \${kpiCard('1','Procurement Cycle Time','','',cycleRag,
      K.kpi1.length?'Overall portfolio average: '+avgCycle+'d (target: '+cycleTgt+'d).':'No cycle data.',
      rptCycBreakdown
    )}
